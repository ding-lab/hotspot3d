package TGI::Mutpro::Preprocess::Prep;
#
#----------------------------------
...
# $Revision: 2016-08-24 $
# $URL: $
# $Doc: run all preprocessing steps $ 
#----------------------------------
#
use strict;
use warnings;
...
sub process {
    ...
    unless (-e $pro_dir) { mkdir($pro_dir) || die "HotSpot3D Prep Error: can not make $pro_dir !\n"; }
    unless (-e $log_file) { if (system("touch $log_file") != 0) { die "HotSpot3D Prep Error: can not make hugo uniprot file !\n"; } }
    unless (-e $inpro_dir) { mkdir($inpro_dir) || die "HotSpot3D Prep Error: can not make $inpro_dir !\n"; }
    unless (-e $pdbcor_dir) { mkdir($pdbcor_dir) || die "HotSpot3D Prep Error: can not make $pdbcor_dir !\n"; }
    unless ( -e $log_dir ) { mkdir( $log_dir ) || die "HotSpot3D Prep Error: can not make $log_dir !\n"; }
	...
	if ( $this->{'genes'} ) { 
		...
		unless( $genesFH->open( $this->{'genes'} , "r" ) ) { die "HotSpot3D Prep Error: Could not open file with genes (".$this->{'genes'}.")"; }
		map {
			chomp;
			@fields = split( "\t" , $_ );
			$list{$fields[0]} = 1;
    ...
    unless( open ( $cmd_list_submit_file_fh, ">", $this->{'cmd_list_submit_file'} ) ) { die "HotSpot3D Prep Error: Could not open cmd file (".$this->{'cmd_list_submit_file'}.")"; }
...
TGI::Mutpro::Preprocess::Prep - Create & update proximity files.

=head1 SYNOPSIS

  use TGI::Mutpro::Preprocess::Prep;

=head1 DESCRIPTION

TGI::Mutpro::Preprocess::Prep is to be used to create & update proximity files.
