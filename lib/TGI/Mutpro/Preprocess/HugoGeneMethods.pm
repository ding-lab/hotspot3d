...
sub makeHugoGeneObjects {
	...
	my $hugo = $entries[1];
	$hugo =~ s/\s+//g;
	$hugoObjects{$hugo} = new TGI::Mutpro::Preprocess::HugoGene;
	$hugoObjects{$hugo}->symbol($hugo);
	(defined $entries[0] && $entries[0] ne "" ) || confess "No ID for '$hugo'";
	# Remove 'HGNC:' prefix in ID
	if ( $entries[0] =~ /HGNC\:(\d+)/ )  { $hugoObjects{$hugo}->id($1); } 
        else { confess "Unexpected format for Hugo id '$entries[0]'"; }
	...
	if ( defined $entries[6] && $entries[6] ne "" ) { foreach $id (split /\,/, $entries[6]) { $hugoObjects{$hugo}->addOldDescription($id); } }
	if ( defined $entries[8] && $entries[8] ne "" ) {
	    foreach $id (split /\,/, $entries[6]) { $hugoObjects{$hugo}->addOldDescription($id); } 
	}
