package TGI::Mutpro::Preprocess::Visual;
#
#----------------------------------
# $Authors: Beifang Niu 
...
sub process {
    ...
    unless( $this->{_PROXIMITY_FILE} ) { 
        warn 'You must provide a proximity file ! ', "\n";
        die $this->help_text();
    }
    unless( -e $this->{_PROXIMITY_FILE} ) {
        warn ' proximity file is not exist  ! ', "\n";
        die $this->help_text();
    }
    my @t = qw( num_muts pdb pairs );
...
sub markMutations {
    ...
    foreach my $pdbid ( keys %$mutstoshowf ) {
        ...
        my $pcmd = "$PyMol $tf -x -c -W 800 -H 600 -d \'set internal_gui=0; show_as cartoon; spectrum b;' ";

        foreach my $iterchain ( keys %{$mutstoshowf->{$pdbid}} ) {
            foreach my $iterloc ( keys %{$mutstoshowf->{$pdbid}->{$iterchain}} ) {
                my $aacs = "";
                foreach my $iteraac ( keys %{$mutstoshowf->{$pdbid}->{$iterchain}->{$iterloc}} ) {
                    $aacs .= $iteraac.'|';
                }
                chop($aacs) if ($aacs);
                my $one_mut = ""; 
                if ( $this->{_NO_LABEL} ) {
                    $one_mut = " -d 'sele t$i, (resi $iterloc and chain $iterchain); color $this->{_MUT_COLOR}, selection=t$i; show $this->{_MUT_STYLE}, selection=t$i;'";
                } else {
                    $one_mut = " -d 'sele t$i, (resi $iterloc and chain $iterchain); color $this->{_MUT_COLOR}, selection=t$i; show $this->{_MUT_STYLE}, selection=t$i; set label_font_id, 10; set label_position, (3,2,1); set label_size, 20; label t$i and name CA, \"$aacs\";'";
                }
                $pcmd .= $one_mut;
                $i++;
            }
...
--output-dir		Output directory of scripts for PyMol
