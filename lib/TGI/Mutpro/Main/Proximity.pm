...
sub process {
    ...
    my $drugport_file = "$this->{_DATA_DIR}\/drugport/drugs_pdbs_parsing_list_HotSpot3D_only_pdb_parse_final_results";
    ...
    my $drugport_hash_ref = $this->getDrugportInfo( $drugport_file );
    my $mafHashRef = $this->parseMaf( $this->{_MAF}, $transToUniprot );
    $this->{_STAT}{'num_uniprot_involved'} = keys %$mafHashRef;
    my ($pairoutref, $cosmicref, $roiref, $drugport_results_ref, $drugport_nonresults_ref ) = $this->proximitySearching( $mafHashRef, $prior_dir, $drugport_hash_ref );
...
sub proximitySearching {
    ...
    foreach my $a ( keys %$mafHashref ) {
        ...
        while ( my $b = <$fh> ) {
            ...
            my ( $real_chain1 ) = $chain1 =~ /\[(\w)\]/;
            my ( $real_chain2 ) = $chain2 =~ /\[(\w)\]/;
            my ( $e, $iter );
            map { $iter = $_;
                if ( defined $drugportref->{'TARGET'}->{$iter}->{$real_chain1}->{$pdbcor1} ) {
                    if ( defined $mafHashref->{$uid2}->{$uniprotcor2} ) {
                        foreach $e ( keys %{$mafHashref->{$uid2}->{$uniprotcor2}} ) {
                            push( @drugport_target_results, join("\t", $iter, $chain1, $pdbcor1, $e, @ta[8,9,12,13], $lineardis, $proximityinfor) );
                        }
                    }
                }
                if ( defined $drugportref->{'TARGET'}->{$iter}->{$real_chain2}->{$pdbcor2} ) {
                    if ( defined $mafHashref->{$uid1}->{$uniprotcor1} ) {
                        foreach $e ( keys %{$mafHashref->{$uid1}->{$uniprotcor1}} ) {
                            push( @drugport_target_results, join("\t", $iter, $chain2, $pdbcor2, $e, @ta[1,2,5,6], $lineardis, $proximityinfor) );
                        }
                    }
                }
                if ( defined $drugportref->{'NONTARGET'}->{$iter}->{$real_chain1}->{$pdbcor1} ) {
                    if ( defined $mafHashref->{$uid2}->{$uniprotcor2} ) {
                        foreach $e ( keys %{$mafHashref->{$uid2}->{$uniprotcor2}} ) {
                            push( @drugport_nontarget_results, join("\t", $iter, $chain1, $pdbcor1, $e, @ta[8,9,12,13], $lineardis, $proximityinfor) );
                        }
                    }
                }
                if ( defined $drugportref->{'NONTARGET'}->{$iter}->{$real_chain2}->{$pdbcor2} ) {
                    if ( defined $mafHashref->{$uid1}->{$uniprotcor1} ) {
                        foreach $e ( keys %{$mafHashref->{$uid1}->{$uniprotcor1}} ) {
                            push( @drugport_nontarget_results, join("\t", $iter, $chain2, $pdbcor2, $e, @ta[1,2,5,6], $lineardis, $proximityinfor) );
                        }
                    }
                }
            } keys %pdbs_hash;
