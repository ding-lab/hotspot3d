...
sub writeProximityFile {
    ...
    %pdbIds = ();
    if ( !defined $pdbRef || scalar(@{$pdbRef}) == 0 ) { carp "Did not get pdb IDs for $this->{'uniprot_id'} \n"; return; }
	...
	if ( $this->{'restrictedaa_pairs'} ) { $uniprotIdToDomainsRef = getDomainsForAllUniprotIds( $chainToUniprotIdRef ); }
	...
	foreach $residuePosition ( @{$residuePositionsRef} ) {
		...
		if ( defined $$uniprotDomainRef{$residuePosition+$uniprotChainOffset} ) {
			...
			print $fh "$this->{'uniprot_id'}\t[$uniprotChain]\t$residuePosition\t$uniprotChainOffset\t",
                        "$uniprotAaName\t$this->{'uniprot_id'}\t[$uniprotChain]\t$residuePosition\t",
		...
		if ( $this->{'restrictedaa_pairs'} ) { $otherDomainRef = $$uniprotIdToDomainsRef{ $otherChainUniprotId }; }
		...
		foreach $position ( sort {$a<=>$b} @tmp_array_positions ) {
		    ...
		    if ( ! $this->{'restrictedaa_pairs'} ) { $fh->print( "$this->{'uniprot_id'}\t[$uniprotChain]\t$residuePosition\t$uniprotChainOffset\t$uniprotAaName\t$otherChainUniprotId\t[$chain]\t$position\t$otherChainOffset\t$otherAaName\t$distanceBetweenResidues\t$pdbId\n" ); }
			...
			if ( $otherChainUniprotId ne $this->{'uniprot_id'} ) {
			    ...
			    foreach ( @otherChainDomains ) { $fh->print( "$this->{'uniprot_id'}\t[$uniprotChain]\t$residuePosition\t$uniprotChainOffset\t$uniprotAaName\t$otherChainUniprotId\t[$chain]\t$position\t$otherChainOffset\t$otherAaName\t$_\t$distanceBetweenResidues\t$pdbId\n"); }
                            next;
			}
			...
			foreach my $domain ( keys %{$$otherDomainRef{$correctedPosition}} ) { 
			    ...
			    $fh->print( "$this->{'uniprot_id'}\t[$uniprotChain]\t$residuePosition\t$uniprotChainOffset\t$uniprotAaName\t$otherChainUniprotId\t[$chain]\t$position\t$otherChainOffset\t$otherAaName\t$domain\t$distanceBetweenResidues\t$pdbId\n" );
			}
		    }
                }
            }
